...
7. **The `action` field MUST be price-conditional, never generic Buy/Sell.** A standalone "Buy" or "Sell" rating without a price anchor is misleading because it suggests the call holds at any price. Every action field must follow this pattern: `"At $<current price>: <BUY / HOLD / TRIM / SELL / WAIT>. <One-line why>. Next trigger at $<next-action price>. Stop $<stop>."` Example: ❌ `"Buy"` → ✅ `"At $129.44: BUY (Tranche 1 zone $125-$130). Tranche 2 trigger at $115-$120 retest. Stop $95."` This applies equally to short-side actions: ❌ `"Sell"` → ✅ `"At $445: TRIM (Tranche 1 trim zone $440-$455 active). Bear-put spread $420/$360 tactical. Stop $470."`. **The structural rating tier (Buy/Overweight/Hold/Underweight/Sell) stays as the long-term directional view; the `action` field is the price-conditional operational call.** A "Buy"-rated stock at the top of its tranche zone is still HOLD or even TRIM in the action field — that distinction is the whole point of the tranche framework and must not be collapsed.

8. **Every daily action recommendation MUST be re-derived from today's evidence, NOT inherited from yesterday.** This is the binding interpretation of SKILL.md's "fact-driven, history-anchored, first-principles" philosophy. "History-anchored" means **prior decisions are reference material (like a prior analyst note you might cite)**, NOT a decision anchor that the new call inherits from. Concrete implementation:
   * ❌ "Yesterday I said HOLD; small price change today; let me update HOLD with new strikes" — this is decision-inheritance disguised as refresh
   * ✅ "Today's evidence: <price + cohort + cross-asset + news>. Re-running Phase 2 (Bull vs Bear) + Phase 5 (Aggressive/Conservative/Neutral risk) from scratch. Synthesis says: <new action>." — this is independent re-derivation
   * The `JSON.parse(JSON.stringify(prior-date))` clone pattern in `data.js` is FOR DATA STRUCTURE INHERITANCE ONLY (ticker list, category taxonomy, market brief skeleton). It MUST NOT be a shortcut to inheriting prior action recommendations. Every ticker's action field on date X must be freshly derived from date X evidence, even when the rating tier and price are similar to date X-1.
   * Why this matters: a decision that "made sense yesterday" often stops making sense when re-derived from today's full evidence set. Inheritance creates path-dependence and prevents the dialectic from surfacing weaknesses. The user has explicitly flagged this anti-pattern: "操作建议需要全部每天自己写，之前的所有 history 只是一个 reference" (action recommendations must be written fresh every day; all prior history is just a reference).
   * Operational test: if the action field on date X is materially identical in framing to date X-1's action field, you almost certainly inherited rather than re-derived. The check: did you run a fresh Phase 2 dialectic and Phase 5 risk debate today, or did you just patch yesterday's call with new prices? If the latter, redo it.

9. **The 80/20 HARD STANDARD: 80% of analytical effort = fresh multi-agent debate output; 20% = historical reference.** Every ticker's body and action on every daily refresh must be derivable from this composition. Concrete operationalization:
   * **80% multi-agent debate** = the Phase 2 Bull-vs-Bear + Phase 5 Aggressive/Conservative/Neutral risk debate is the PRIMARY source of analytical content. The Bull/Bear/Risk subagent outputs are NOT supplementary commentary on top of a pre-formed thesis — they ARE the analytical thesis. My synthesis (Research Manager Phase 3 + Portfolio Manager Phase 6) takes the debate outputs as input and reweighs them; it does NOT generate independent thesis content outside of the debate.
   * **20% historical reference** = prior dates' bodies, prior decisions, prior price/event data are CITED for context but are NOT inheritable conclusions. Format: "Prior date 2026-05-X noted: <quote>. Today's debate reweighs: <new synthesis>." The prior thesis must be explicitly stated and explicitly reweighed, not silently carried forward.
   * **Implementation discipline**: every ticker's daily Phase 2 section must visibly reference what 3+ distinct subagent contexts (Bull, Bear, Neutral minimum) said, with each side's data points attributed. If the Phase 2 reads as a single internally-consistent narrative, it's single-voice writing dressed as multi-agent and fails the 80/20 test.
   * **Hard test**: take any daily ticker body and ask: "Can I identify 5+ specific data points that came from independent subagent contexts (not from my own pre-formed view)?" If no, the 80/20 standard fails and the body must be rewritten.
